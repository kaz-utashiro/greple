...
is(greple(q(-e "青いそら" --only t/JA.txt))->result,
     "青いそら\n", "--only");

is(greple(q(-e "イーハトーヴォ" --only t/JA.txt))->result,
     "イーハトーヴォ\n" x 2, "--only (2)");

is(greple(q(-e "イーハトーヴォ" --only --max 1 t/JA.txt))->result,
     "イーハトーヴォ\n", "--max 1");

like(greple(q(-e "山猫博士" --context t/JA.txt))->result,
     qr/\A(.*\n){5}--\n\z/, "--context");

like(greple(q(-e "山猫博士" --after=1 t/JA.txt))->result,
     qr/\A(山猫博士.*\n)(.*\n)--\n\z/, "--after=1");

like(greple(q(-e "山猫博士" --before=1 t/JA.txt))->result,
     qr/\A(.*\n)(山猫博士.*\n)--\n\z/, "--before=1");
